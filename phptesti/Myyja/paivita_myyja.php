<?php
    ...
    // Else haarassa haetaan myyjaID parametrin mukaiset myyjatiedot tietokannasta
    if ( !empty($_POST)) {
        ...
        if ($valid) {
            ...
            $sql = "UPDATE myyja SET etunimi = ?, sukunimi = ?, lahiosoite = ?, postinumero = ?, postitoimipaikka = ?, kayttajatunnus = ?, salasana = ?, rooli = ? WHERE myyjaID = ?";
            $q = $pdo->prepare($sql);
            $q->execute(array($etunimi, $sukunimi, $lahiosoite, $postinumero, $postitoimipaikka, $kayttajatunnus, $salasana, $rooli, $myyjaID));
            Database::disconnect();
            header("Location: myyja.php");
        }
    }  else {
...
<html lang="fi">
  ...
  <body>
    ...
    <div class="container">    
        ...
        <!--    Lomake myyjatietojen päivittämistä varten.
                Koodissa käytetään lyhennettyä if-else -lausetta (esim. !empty($kayttajatunnusError)?'alert alert-info':'';)
                Koodissa käytetään myös if-lausetta joka päättyy kohdassa endif
                Huomioi, että formin action kentässä välitetään myös myyjaID parametri
        -->
