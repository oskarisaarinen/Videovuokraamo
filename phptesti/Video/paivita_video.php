<?php
    ...
    $videoID = null; // Alustetaan muuttuja.
    ...
    // Jos videoID parametriä ei välitetty, palautetaan käyttäjä takaisin video.php sivulle
    if ( null==$videoID ) {
    ...
    // Else haarassa haetaan videoID parametrin mukaiset video tietokannasta
    if ( !empty($_POST)) {
        ...
        if ($valid) {
            ...
            $sql = "UPDATE video SET nimi = ?, genre = ?, kuvaus = ?, ikaraja = ?, kesto = ?, julkaisupaiva = ?, tuotantovuosi = ?, ohjaaja = ?, nayttelijat = ?, kuva = ? WHERE videoID = ?";
            $q = $pdo->prepare($sql);
            $q->execute(array($nimi, $genre, $kuvaus, $ikaraja, $kesto, $julkaisupaiva, $tuotantovuosi, $ohjaaja, $nayttelijat, $kuvat, $videoID));
            Database::disconnect();
            header("Location: video.php");
        }
    }  else {
...
<html lang="fi">
  ...
  <body>
    ...
    <div class="container">    
        ...
        <!--    Lomake videotietojen päivittämistä varten.
                Koodissa käytetään lyhennettyä if-else -lausetta (esim. !empty($nimiError)?'alert alert-info':'';)
                Koodissa käytetään myös if-lausetta joka päättyy kohdassa endif
                Huomioi, että formin action kentässä välitetään myös videoID parametri
        -->
